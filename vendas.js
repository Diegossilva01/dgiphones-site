...
const CACHE_PRODUTOS_KEY='celltech_produtos_v1';
function aplicarProdutos(lista){produtos=Array.isArray(lista)?lista:[];gerarFiltros();render(produtos)}
function lerCacheProdutos(){try{const x=JSON.parse(localStorage.getItem(CACHE_PRODUTOS_KEY)||'null');return x&&Array.isArray(x.produtos)?x:null}catch(e){return null}}
function salvarCacheProdutos(lista){try{localStorage.setItem(CACHE_PRODUTOS_KEY,JSON.stringify({ts:Date.now(),produtos:lista}))}catch(e){}}
async function carregar(){
  const c=document.getElementById('produtos'),cache=lerCacheProdutos();
  if(cache)aplicarProdutos(cache.produtos);
  try{
    const r=await fetch(API_URL+'?acao=produtosSite',{cache:'default'});
    if(!r.ok)throw new Error('HTTP '+r.status);
    const j=await r.json();
    if(!j.sucesso)throw new Error(j.mensagem||'Erro');
    salvarCacheProdutos(j.produtos||[]);
    aplicarProdutos(j.produtos||[]);
  }catch(e){
    if(!cache)c.innerHTML='<p>Não foi possível carregar os produtos.</p>';
  }
}
