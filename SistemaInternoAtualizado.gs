...
function listarPeriodo_(nomeAba,campoData,d,camposBusca){
  ...
  const h=vals[0];
  const idxData=h.indexOf(campoData);
    ...
    .filter(r=>{
      ...
      if(q){
        const achou=camposBusca.some(c=>{
          ...
          return idx>=0&&String(r[idx]||'').toLowerCase().includes(q);
        });
...
// Converte qualquer data da planilha ou do site para AAAA-MM-DD.
// Isso evita falhas de filtro causadas por horário, localidade ou fuso.
