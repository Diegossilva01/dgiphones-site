...
function atualizarProdutoSite_(d){
  auth_(d.token);const sh=aba_(CONFIG.ABAS.ESTOQUE),v=sh.getDataRange().getValues();let row=-1;
  for(let i=1;i<v.length;i++)if(String(v[i][0])===txt_(d.idEstoque)){row=i+1;break}if(row<0)throw new Error('Produto não encontrado.');
  ...
  if(d.statusSite!==undefined){const st=txt_(d.statusSite);if(!['Publicado','Oculto','Reservado'].includes(st))throw new Error('Status inválido.');if(txt_(sh.getRange(row,10).getValue()).toLowerCase()==='vendido')throw new Error('Produto já vendido.');sh.getRange(row,18).setValue(st)}
  return{sucesso:true};
}
...
function editarCompra_(d){
  ...
  return{sucesso:true,mensagem:'Compra alterada com sucesso.'};
}
