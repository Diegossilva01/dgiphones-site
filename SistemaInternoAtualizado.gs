...
function listarCompras_(d){auth_(d.token);return{sucesso:true,compras:listarPeriodo_(CONFIG.ABAS.COMPRAS,'Data da compra',d,['Nome do vendedor','CPF','Telefone','Modelo','IMEI','IMEI 2']).reverse()}}
function listarVendas_(d){auth_(d.token);return{sucesso:true,vendas:listarPeriodo_(CONFIG.ABAS.VENDAS,'Data da venda',d,['Nome do cliente','CPF','Telefone','Modelo','IMEI','IMEI 2','Funcionário responsável']).reverse()}}
function listarPeriodo_(nomeAba,campoData,d,camposBusca){
  const sh=aba_(nomeAba),vals=sh.getDataRange().getValues();if(vals.length<2)return[];
  const h=vals[0],idxData=h.indexOf(campoData),ini=d.dataInicial?inicio_(data_(d.dataInicial)):null,fim=d.dataFinal?fim_(data_(d.dataFinal)):null,q=txt_(d.pesquisa).toLowerCase();
  return vals.slice(1).filter(r=>r.some(v=>v!=='' )).filter(r=>{
    const dt=r[idxData] instanceof Date?r[idxData]:parseDataBR_(r[idxData]);if(!dt)return false;if(ini&&dt<ini)return false;if(fim&&dt>fim)return false;
    if(q&&!camposBusca.some(c=>String(r[h.indexOf(c)]||'').toLowerCase().includes(q)))return false;return true;
  }).map(r=>obj_(h,r));
}
function listarEstoque_(d){auth_(d.token);const regs=registros_(aba_(CONFIG.ABAS.ESTOQUE));const st=txt_(d.status).toLowerCase(),q=txt_(d.pesquisa).toLowerCase();return{sucesso:true,estoque:regs.filter(r=>(!st||txt_(r.Status).toLowerCase()===st)&&(!q||[r.Modelo,r.Armazenamento,r.Cor,r.IMEI,r['IMEI 2']].some(v=>String(v||'').toLowerCase().includes(q)))).reverse()}}

function dashboard_(d){auth_(d.token);const vendas=registrosRaw_(aba_(CONFIG.ABAS.VENDAS)),compras=registrosRaw_(aba_(CONFIG.ABAS.COMPRAS)),estoque=registros_(aba_(CONFIG.ABAS.ESTOQUE)),hoje=inicio_(new Date()),mes=new Date(hoje.getFullYear(),hoje.getMonth(),1);
  const vh=vendas.filter(x=>mesma_(x['Data da venda'],hoje)),vm=vendas.filter(x=>dataSeg_(x['Data da venda'])>=mes),ch=compras.filter(x=>mesma_(x['Data da compra'],hoje)),cm=compras.filter(x=>dataSeg_(x['Data da compra'])>=mes),disp=estoque.filter(x=>txt_(x.Status).toLowerCase()==='disponível');
  return{sucesso:true,resumo:{faturamentoHoje:soma_(vh,'Valor da venda'),faturamentoMes:soma_(vm,'Valor da venda'),lucroHoje:soma_(vh,'Lucro'),lucroMes:soma_(vm,'Lucro'),quantidadeVendasHoje:vh.length,quantidadeVendasMes:vm.length,comprasHoje:ch.length,comprasMes:cm.length,valorComprasHoje:soma_(ch,'Valor da compra'),valorComprasMes:soma_(cm,'Valor da compra'),aparelhosDisponiveis:disp.length,valorEstoque:soma_(disp,'Valor de custo'),ticketMedioMes:vm.length?soma_(vm,'Valor da venda')/vm.length:0},grafico:grafico_(vendas)};
}
...
function criarAba_(ss,nome,h){let sh=ss.getSheetByName(nome);if(!sh)sh=ss.insertSheet(nome);if(sh.getLastRow()===0)sh.getRange(1,1,1,h.length).setValues([h]);sh.setFrozenRows(1)}
