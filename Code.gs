...
function doPost(e){
  try{
    ...
    if(!['login','verificarToken','logout'].includes(acao)){const usuario=auth_(d.token);if(txt_(usuario.perfil).toLowerCase()==='revendedor'&&acao!=='listarCatalogoRevendedor')throw new Error('Seu acesso é exclusivo ao catálogo de revenda.');}
    let r;
    switch(acao){
...
function funcionarioLinha_(r,m){return{id:r[m['id']],nome:r[m['nome']],usuario:r[m['usuário']]??r[m['usuario']],perfil:r[m['perfil']]||'Funcionário',status:r[m['status']]||'Ativo'}}
function login_(d){
...
function listarFuncionarios_(d){
  ...
  const funcionarios=vals.slice(1).filter(r=>r.some(x=>x!=='' )).map(r=>{const f=funcionarioLinha_(r,m);return{id:f.id,nome:f.nome,usuario:f.usuario,perfil:f.perfil,status:f.status,dataCadastro:r[m['data de cadastro']] instanceof Date?fmtDH_(r[m['data de cadastro']]):txt_(r[m['data de cadastro']])}});
  return{sucesso:true,funcionarios};
}

function cadastrarFuncionario_(d){const u=auth_(d.token);admin_(u);if(!txt_(d.nome)||txt_(d.usuario).length<3||String(d.senha||'').length<6)throw new Error('Confira os dados do funcionário.');const sh=aba_(CONFIG.ABAS.FUNCIONARIOS),vals=sh.getDataRange().getValues();if(vals.slice(1).some(r=>txt_(r[2]).toLowerCase()===txt_(d.usuario).toLowerCase()))throw new Error('Este usuário já existe.');sh.appendRow([id_('FUNC'),txt_(d.nome),txt_(d.usuario).toLowerCase(),hash_(String(d.senha)),['Administrador','Revendedor'].includes(txt_(d.perfil))?txt_(d.perfil):'Funcionário',txt_(d.status)==='Inativo'?'Inativo':'Ativo',new Date()]);return{sucesso:true}}
