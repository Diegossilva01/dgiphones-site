...
function abrirEditor(tipo,r){
  ...
  modal.innerHTML=`<div class="edit-card"><div class="edit-head"><h2>Editar ${venda?'venda':'compra'}</h2><button type="button" class="edit-close">×</button></div><form id="editRegistroForm" class="grid-form">
  ...
  modal.querySelector('form').onsubmit=async e=>{e.preventDefault();const f=e.currentTarget,d=formData(f);d[venda?'valorVenda':'valorCompra']=parseMoney(d[venda?'valorVenda':'valorCompra']);d[venda?'idVenda':'idCompra']=r[venda?'ID Venda':'ID Compra'];setMsg(f,'Salvando...');try{await api(venda?'editarVenda':'editarCompra',d);modal.remove();toast('Registro alterado');state.dados=null;loadHistorico();loadEstoque();loadDashboard();loadComissao()}catch(err){setMsg(f,err.message)}};
}
